...
class LeadController extends Controller
{
    ...
    public function view(int $id)
    {
        ...
        // --- BĐS Suggestion Logic ---
        $query = \Webkul\Product\Models\Product::query();

        if ($lead->demand_budget_from) {
            $query->where('price', '>=', $lead->demand_budget_from);
        }
        if ($lead->demand_budget_to) {
            $query->where('price', '<=', $lead->demand_budget_to);
        }

        $existingProductIds = $lead->products->pluck('product_id')->toArray();
        if (! empty($existingProductIds)) {
            $query->whereNotIn('id', $existingProductIds);
        }

        $suggestedProducts = $query->take(20)->get();

        $options = app(\Webkul\Attribute\Repositories\AttributeOptionRepository::class)->all()->keyBy('id');

        $suggestedProductsMapped = $suggestedProducts->map(function ($product) use ($options) {
            return [
                'id'           => $product->id,
                'name'         => $product->name,
                'sku'          => $product->sku,
                'price'        => $product->price,
                'dien_tich_m2' => $product->dien_tich_m2 ?: '-',
                'loai_bds'     => isset($options[$product->loai_bds]) ? $options[$product->loai_bds]->name : '-',
                'vi_tri'       => isset($options[$product->vi_tri_quan]) ? $options[$product->vi_tri_quan]->name : '-',
            ];
        });

        $suggestedProducts = $suggestedProductsMapped;

        return view('admin::leads.view', compact('lead', 'suggestedProducts'));
    }
    ...
    public function updateStage(int $id)
    {
        ...
        if ($stage->code === 'won') {
            \App\Models\CommissionSplit::firstOrCreate([
                'lead_id' => $lead->id,
                'user_id' => $lead->user_id, // Default to the lead assignee
            ], [
                'role'                  => 'sale',
                'commission_percentage' => 3.0, // Default 3% percent default
                'commission_amount'     => $lead->lead_value ? ($lead->lead_value * 0.03) : 0,
                'status'                => 'pending',
            ]);
        }
